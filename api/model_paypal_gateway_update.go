/*
Commerce Layer API

Headless Commerce for Global Brands.

API version: 7.6.1
Contact: support@commercelayer.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// checks if the PaypalGatewayUpdate type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PaypalGatewayUpdate{}

// PaypalGatewayUpdate struct for PaypalGatewayUpdate
type PaypalGatewayUpdate struct {
	Data PaypalGatewayUpdateData `json:"data"`
}

// NewPaypalGatewayUpdate instantiates a new PaypalGatewayUpdate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPaypalGatewayUpdate(data PaypalGatewayUpdateData) *PaypalGatewayUpdate {
	this := PaypalGatewayUpdate{}
	this.Data = data
	return &this
}

// NewPaypalGatewayUpdateWithDefaults instantiates a new PaypalGatewayUpdate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPaypalGatewayUpdateWithDefaults() *PaypalGatewayUpdate {
	this := PaypalGatewayUpdate{}
	return &this
}

// GetData returns the Data field value
func (o *PaypalGatewayUpdate) GetData() PaypalGatewayUpdateData {
	if o == nil {
		var ret PaypalGatewayUpdateData
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *PaypalGatewayUpdate) GetDataOk() (*PaypalGatewayUpdateData, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Data, true
}

// SetData sets field value
func (o *PaypalGatewayUpdate) SetData(v PaypalGatewayUpdateData) {
	o.Data = v
}

func (o PaypalGatewayUpdate) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PaypalGatewayUpdate) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	return toSerialize, nil
}

type NullablePaypalGatewayUpdate struct {
	value *PaypalGatewayUpdate
	isSet bool
}

func (v NullablePaypalGatewayUpdate) Get() *PaypalGatewayUpdate {
	return v.value
}

func (v *NullablePaypalGatewayUpdate) Set(val *PaypalGatewayUpdate) {
	v.value = val
	v.isSet = true
}

func (v NullablePaypalGatewayUpdate) IsSet() bool {
	return v.isSet
}

func (v *NullablePaypalGatewayUpdate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePaypalGatewayUpdate(val *PaypalGatewayUpdate) *NullablePaypalGatewayUpdate {
	return &NullablePaypalGatewayUpdate{value: val, isSet: true}
}

func (v NullablePaypalGatewayUpdate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePaypalGatewayUpdate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

/*
Commerce Layer API

Headless Commerce for Global Brands.

API version: 7.6.1
Contact: support@commercelayer.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// checks if the ShippingWeightTierUpdate type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ShippingWeightTierUpdate{}

// ShippingWeightTierUpdate struct for ShippingWeightTierUpdate
type ShippingWeightTierUpdate struct {
	Data ShippingWeightTierUpdateData `json:"data"`
}

// NewShippingWeightTierUpdate instantiates a new ShippingWeightTierUpdate object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewShippingWeightTierUpdate(data ShippingWeightTierUpdateData) *ShippingWeightTierUpdate {
	this := ShippingWeightTierUpdate{}
	this.Data = data
	return &this
}

// NewShippingWeightTierUpdateWithDefaults instantiates a new ShippingWeightTierUpdate object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewShippingWeightTierUpdateWithDefaults() *ShippingWeightTierUpdate {
	this := ShippingWeightTierUpdate{}
	return &this
}

// GetData returns the Data field value
func (o *ShippingWeightTierUpdate) GetData() ShippingWeightTierUpdateData {
	if o == nil {
		var ret ShippingWeightTierUpdateData
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *ShippingWeightTierUpdate) GetDataOk() (*ShippingWeightTierUpdateData, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Data, true
}

// SetData sets field value
func (o *ShippingWeightTierUpdate) SetData(v ShippingWeightTierUpdateData) {
	o.Data = v
}

func (o ShippingWeightTierUpdate) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ShippingWeightTierUpdate) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	return toSerialize, nil
}

type NullableShippingWeightTierUpdate struct {
	value *ShippingWeightTierUpdate
	isSet bool
}

func (v NullableShippingWeightTierUpdate) Get() *ShippingWeightTierUpdate {
	return v.value
}

func (v *NullableShippingWeightTierUpdate) Set(val *ShippingWeightTierUpdate) {
	v.value = val
	v.isSet = true
}

func (v NullableShippingWeightTierUpdate) IsSet() bool {
	return v.isSet
}

func (v *NullableShippingWeightTierUpdate) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableShippingWeightTierUpdate(val *ShippingWeightTierUpdate) *NullableShippingWeightTierUpdate {
	return &NullableShippingWeightTierUpdate{value: val, isSet: true}
}

func (v NullableShippingWeightTierUpdate) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableShippingWeightTierUpdate) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

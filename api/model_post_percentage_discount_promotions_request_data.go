/*
Commerce Layer API

Headless Commerce for Global Brands.

API version: 4.1.3
Contact: support@commercelayer.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// checks if the POSTPercentageDiscountPromotionsRequestData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &POSTPercentageDiscountPromotionsRequestData{}

// POSTPercentageDiscountPromotionsRequestData struct for POSTPercentageDiscountPromotionsRequestData
type POSTPercentageDiscountPromotionsRequestData struct {
	// The resource's type
	Type          interface{}                                                             `json:"type"`
	Attributes    POSTPercentageDiscountPromotionsRequestDataAttributes                   `json:"attributes"`
	Relationships *PATCHFixedPricePromotionsFixedPricePromotionIdRequestDataRelationships `json:"relationships,omitempty"`
}

// NewPOSTPercentageDiscountPromotionsRequestData instantiates a new POSTPercentageDiscountPromotionsRequestData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPOSTPercentageDiscountPromotionsRequestData(type_ interface{}, attributes POSTPercentageDiscountPromotionsRequestDataAttributes) *POSTPercentageDiscountPromotionsRequestData {
	this := POSTPercentageDiscountPromotionsRequestData{}
	this.Type = type_
	this.Attributes = attributes
	return &this
}

// NewPOSTPercentageDiscountPromotionsRequestDataWithDefaults instantiates a new POSTPercentageDiscountPromotionsRequestData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPOSTPercentageDiscountPromotionsRequestDataWithDefaults() *POSTPercentageDiscountPromotionsRequestData {
	this := POSTPercentageDiscountPromotionsRequestData{}
	return &this
}

// GetType returns the Type field value
// If the value is explicit nil, the zero value for interface{} will be returned
func (o *POSTPercentageDiscountPromotionsRequestData) GetType() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *POSTPercentageDiscountPromotionsRequestData) GetTypeOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Type) {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *POSTPercentageDiscountPromotionsRequestData) SetType(v interface{}) {
	o.Type = v
}

// GetAttributes returns the Attributes field value
func (o *POSTPercentageDiscountPromotionsRequestData) GetAttributes() POSTPercentageDiscountPromotionsRequestDataAttributes {
	if o == nil {
		var ret POSTPercentageDiscountPromotionsRequestDataAttributes
		return ret
	}

	return o.Attributes
}

// GetAttributesOk returns a tuple with the Attributes field value
// and a boolean to check if the value has been set.
func (o *POSTPercentageDiscountPromotionsRequestData) GetAttributesOk() (*POSTPercentageDiscountPromotionsRequestDataAttributes, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Attributes, true
}

// SetAttributes sets field value
func (o *POSTPercentageDiscountPromotionsRequestData) SetAttributes(v POSTPercentageDiscountPromotionsRequestDataAttributes) {
	o.Attributes = v
}

// GetRelationships returns the Relationships field value if set, zero value otherwise.
func (o *POSTPercentageDiscountPromotionsRequestData) GetRelationships() PATCHFixedPricePromotionsFixedPricePromotionIdRequestDataRelationships {
	if o == nil || IsNil(o.Relationships) {
		var ret PATCHFixedPricePromotionsFixedPricePromotionIdRequestDataRelationships
		return ret
	}
	return *o.Relationships
}

// GetRelationshipsOk returns a tuple with the Relationships field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *POSTPercentageDiscountPromotionsRequestData) GetRelationshipsOk() (*PATCHFixedPricePromotionsFixedPricePromotionIdRequestDataRelationships, bool) {
	if o == nil || IsNil(o.Relationships) {
		return nil, false
	}
	return o.Relationships, true
}

// HasRelationships returns a boolean if a field has been set.
func (o *POSTPercentageDiscountPromotionsRequestData) HasRelationships() bool {
	if o != nil && !IsNil(o.Relationships) {
		return true
	}

	return false
}

// SetRelationships gets a reference to the given PATCHFixedPricePromotionsFixedPricePromotionIdRequestDataRelationships and assigns it to the Relationships field.
func (o *POSTPercentageDiscountPromotionsRequestData) SetRelationships(v PATCHFixedPricePromotionsFixedPricePromotionIdRequestDataRelationships) {
	o.Relationships = &v
}

func (o POSTPercentageDiscountPromotionsRequestData) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o POSTPercentageDiscountPromotionsRequestData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Type != nil {
		toSerialize["type"] = o.Type
	}
	toSerialize["attributes"] = o.Attributes
	if !IsNil(o.Relationships) {
		toSerialize["relationships"] = o.Relationships
	}
	return toSerialize, nil
}

type NullablePOSTPercentageDiscountPromotionsRequestData struct {
	value *POSTPercentageDiscountPromotionsRequestData
	isSet bool
}

func (v NullablePOSTPercentageDiscountPromotionsRequestData) Get() *POSTPercentageDiscountPromotionsRequestData {
	return v.value
}

func (v *NullablePOSTPercentageDiscountPromotionsRequestData) Set(val *POSTPercentageDiscountPromotionsRequestData) {
	v.value = val
	v.isSet = true
}

func (v NullablePOSTPercentageDiscountPromotionsRequestData) IsSet() bool {
	return v.isSet
}

func (v *NullablePOSTPercentageDiscountPromotionsRequestData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePOSTPercentageDiscountPromotionsRequestData(val *POSTPercentageDiscountPromotionsRequestData) *NullablePOSTPercentageDiscountPromotionsRequestData {
	return &NullablePOSTPercentageDiscountPromotionsRequestData{value: val, isSet: true}
}

func (v NullablePOSTPercentageDiscountPromotionsRequestData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePOSTPercentageDiscountPromotionsRequestData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

/*
Commerce Layer API

Headless Commerce for Global Brands.

API version: 3.2.0
Contact: support@commercelayer.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// PATCHOrdersOrderId200Response struct for PATCHOrdersOrderId200Response
type PATCHOrdersOrderId200Response struct {
	Data *PATCHOrdersOrderId200ResponseData `json:"data,omitempty"`
}

// NewPATCHOrdersOrderId200Response instantiates a new PATCHOrdersOrderId200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPATCHOrdersOrderId200Response() *PATCHOrdersOrderId200Response {
	this := PATCHOrdersOrderId200Response{}
	return &this
}

// NewPATCHOrdersOrderId200ResponseWithDefaults instantiates a new PATCHOrdersOrderId200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPATCHOrdersOrderId200ResponseWithDefaults() *PATCHOrdersOrderId200Response {
	this := PATCHOrdersOrderId200Response{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *PATCHOrdersOrderId200Response) GetData() PATCHOrdersOrderId200ResponseData {
	if o == nil || o.Data == nil {
		var ret PATCHOrdersOrderId200ResponseData
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PATCHOrdersOrderId200Response) GetDataOk() (*PATCHOrdersOrderId200ResponseData, bool) {
	if o == nil || o.Data == nil {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *PATCHOrdersOrderId200Response) HasData() bool {
	if o != nil && o.Data != nil {
		return true
	}

	return false
}

// SetData gets a reference to the given PATCHOrdersOrderId200ResponseData and assigns it to the Data field.
func (o *PATCHOrdersOrderId200Response) SetData(v PATCHOrdersOrderId200ResponseData) {
	o.Data = &v
}

func (o PATCHOrdersOrderId200Response) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Data != nil {
		toSerialize["data"] = o.Data
	}
	return json.Marshal(toSerialize)
}

type NullablePATCHOrdersOrderId200Response struct {
	value *PATCHOrdersOrderId200Response
	isSet bool
}

func (v NullablePATCHOrdersOrderId200Response) Get() *PATCHOrdersOrderId200Response {
	return v.value
}

func (v *NullablePATCHOrdersOrderId200Response) Set(val *PATCHOrdersOrderId200Response) {
	v.value = val
	v.isSet = true
}

func (v NullablePATCHOrdersOrderId200Response) IsSet() bool {
	return v.isSet
}

func (v *NullablePATCHOrdersOrderId200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePATCHOrdersOrderId200Response(val *PATCHOrdersOrderId200Response) *NullablePATCHOrdersOrderId200Response {
	return &NullablePATCHOrdersOrderId200Response{value: val, isSet: true}
}

func (v NullablePATCHOrdersOrderId200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePATCHOrdersOrderId200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

/*
Commerce Layer API

Headless Commerce for Global Brands.

API version: 3.0.1
Contact: support@commercelayer.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api

import (
	"encoding/json"
)

// PriceResponseDataRelationshipsPriceVolumeTiers struct for PriceResponseDataRelationshipsPriceVolumeTiers
type PriceResponseDataRelationshipsPriceVolumeTiers struct {
	Links *AddressResponseDataRelationshipsGeocoderLinks            `json:"links,omitempty"`
	Data  []PriceResponseDataRelationshipsPriceVolumeTiersDataInner `json:"data,omitempty"`
}

// NewPriceResponseDataRelationshipsPriceVolumeTiers instantiates a new PriceResponseDataRelationshipsPriceVolumeTiers object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPriceResponseDataRelationshipsPriceVolumeTiers() *PriceResponseDataRelationshipsPriceVolumeTiers {
	this := PriceResponseDataRelationshipsPriceVolumeTiers{}
	return &this
}

// NewPriceResponseDataRelationshipsPriceVolumeTiersWithDefaults instantiates a new PriceResponseDataRelationshipsPriceVolumeTiers object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPriceResponseDataRelationshipsPriceVolumeTiersWithDefaults() *PriceResponseDataRelationshipsPriceVolumeTiers {
	this := PriceResponseDataRelationshipsPriceVolumeTiers{}
	return &this
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) GetLinks() AddressResponseDataRelationshipsGeocoderLinks {
	if o == nil || o.Links == nil {
		var ret AddressResponseDataRelationshipsGeocoderLinks
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) GetLinksOk() (*AddressResponseDataRelationshipsGeocoderLinks, bool) {
	if o == nil || o.Links == nil {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) HasLinks() bool {
	if o != nil && o.Links != nil {
		return true
	}

	return false
}

// SetLinks gets a reference to the given AddressResponseDataRelationshipsGeocoderLinks and assigns it to the Links field.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) SetLinks(v AddressResponseDataRelationshipsGeocoderLinks) {
	o.Links = &v
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) GetData() []PriceResponseDataRelationshipsPriceVolumeTiersDataInner {
	if o == nil || o.Data == nil {
		var ret []PriceResponseDataRelationshipsPriceVolumeTiersDataInner
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) GetDataOk() ([]PriceResponseDataRelationshipsPriceVolumeTiersDataInner, bool) {
	if o == nil || o.Data == nil {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) HasData() bool {
	if o != nil && o.Data != nil {
		return true
	}

	return false
}

// SetData gets a reference to the given []PriceResponseDataRelationshipsPriceVolumeTiersDataInner and assigns it to the Data field.
func (o *PriceResponseDataRelationshipsPriceVolumeTiers) SetData(v []PriceResponseDataRelationshipsPriceVolumeTiersDataInner) {
	o.Data = v
}

func (o PriceResponseDataRelationshipsPriceVolumeTiers) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Links != nil {
		toSerialize["links"] = o.Links
	}
	if o.Data != nil {
		toSerialize["data"] = o.Data
	}
	return json.Marshal(toSerialize)
}

type NullablePriceResponseDataRelationshipsPriceVolumeTiers struct {
	value *PriceResponseDataRelationshipsPriceVolumeTiers
	isSet bool
}

func (v NullablePriceResponseDataRelationshipsPriceVolumeTiers) Get() *PriceResponseDataRelationshipsPriceVolumeTiers {
	return v.value
}

func (v *NullablePriceResponseDataRelationshipsPriceVolumeTiers) Set(val *PriceResponseDataRelationshipsPriceVolumeTiers) {
	v.value = val
	v.isSet = true
}

func (v NullablePriceResponseDataRelationshipsPriceVolumeTiers) IsSet() bool {
	return v.isSet
}

func (v *NullablePriceResponseDataRelationshipsPriceVolumeTiers) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePriceResponseDataRelationshipsPriceVolumeTiers(val *PriceResponseDataRelationshipsPriceVolumeTiers) *NullablePriceResponseDataRelationshipsPriceVolumeTiers {
	return &NullablePriceResponseDataRelationshipsPriceVolumeTiers{value: val, isSet: true}
}

func (v NullablePriceResponseDataRelationshipsPriceVolumeTiers) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePriceResponseDataRelationshipsPriceVolumeTiers) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
